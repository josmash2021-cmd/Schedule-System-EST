...
function cargarHistoriales() {
  try {
    ...
    for (const [jid, sesion] of datos) {
      ...
      // Refrescar el system prompt (lleva la fecha y hora actual), con el
      // canal de la sesión (Instagram por defecto).
      if (sesion.mensajes[0]?.role === 'system') {
        sesion.mensajes[0] = { role: 'system', content: construirSystemPrompt(sesion.canalTipo, personaParaChat(jid).nombre) };
      }
 ...
 */
export function construirSystemPrompt(canalTipo = 'instagram', nombrePersona = 'Ángela') {
  const esAlex = /alex/i.test(nombrePersona);
  const el = esAlex ? 'el' : 'la';
  const encargade = esAlex ? 'el encargado' : 'la encargada';
  const ENCARGADE = esAlex ? 'EL ENCARGADO' : 'LA ENCARGADA';
  const esInstagram = canalTipo === 'instagram';
  const nombreCanal = esInstagram ? 'Instagram Direct' : 'WhatsApp';
...
- Sin formato markdown (ni encabezados, ni tablas, ni listas). ${esInstagram ? 'Texto completamente plano: Instagram Direct no muestra negritas ni ningún formato.' : 'A lo mucho *negritas* de WhatsApp para un dato clave.'}
- Responde SIEMPRE en español.
- NOTAS DE VOZ: si el cliente manda una nota de voz, a ti te llega ya transcrita como texto. Las transcripciones pueden tener errores; si algo no tiene sentido, pide la aclaración con naturalidad ("creo que no te entendí bien, ¿me repites esa parte?"). IMPORTANTE: si la transcripción es confusa, incoherente o claramente mal reconocida, NO inventes una interpretación ni actúes sobre ella (nada de escalar al supervisor, agendar citas o confirmar datos): pide primero la aclaración. RESPONDES EN VOZ ALTA: cuando el cliente te habla por nota de voz, tu respuesta se convierte en audio con tu voz — tiene que sonar como una PERSONA hablando, nunca como alguien leyendo una hoja o una contestadora automática: 1 a 3 frases cortas y sueltas, muletillas naturales con moderación ("mira", "pues", "fíjate"), puntos suspensivos (...) donde una persona pausaría al hablar. Cero enlaces ni datos largos impronunciables en lo hablado. La dirección, teléfonos, enlaces y fotos SIEMPRE se envían por TEXTO aunque la conversación sea de voz (el sistema lo hace solo): si los piden, escribe ese dato igual y menciona que se lo mandas por escrito.
...
- Flujo OBLIGATORIO:
  ...
  4. Confirma con el cliente: nombre completo, motivo de la visita y la hora elegida.${esInstagram ? ' Pide TAMBIÉN su teléfono (obligatorio: Instagram no lo proporciona y el sistema de citas lo requiere).' : ''}
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          telefono: { type: 'string', description: 'Teléfono del cliente (en WhatsApp es opcional: se usa el del chat; en Instagram es OBLIGATORIO, hay que pedírselo al cliente)' },
...
async function ejecutarHerramienta(nombre, args, contexto) {
  ...
  if (nombre === 'crear_cita') {
    ...
    if (contexto.canalTipo === 'instagram' && !telefonoCliente) {
      return 'Falta el teléfono del cliente: en Instagram es un dato OBLIGATORIO para agendar (Instagram no lo proporciona). Pídeselo con naturalidad y luego vuelve a llamar crear_cita con él.';
    }
  ...
  if (nombre === 'enviar_fotos') {
    ...
    const { sock, jid, canal } = contexto;
    const info = config.fotos[clave];
    // Preferir el adaptador de canal (Instagram; ver src/instagram.js); si no
    // hay, fallback al socket de Baileys (compatibilidad con llamadas antiguas).
    if (!canal?.enviarImagen && (!sock || !jid)) {
      return `Simulación: se enviarían ${info.fotos.length} fotos de ${info.nombre} (canal no disponible en esta prueba).`;
    }
    try {
      for (let i = 0; i < info.fotos.length; i++) {
        const caption = i === 0 ? `📸 ${info.nombre}` : undefined;
        if (canal?.enviarImagen) {
          await canal.enviarImagen(jid, info.fotos[i], caption);
        } else {
          await sock.sendMessage(jid, { image: { url: info.fotos[i] }, caption });
        }
 ...
 *
 * @param {string} jid - Identificador de la conversación ('ig:<IGSID>').
 * @param {string} textoUsuario - Mensaje de texto del usuario.
 * @param {{telefono: string, sock: object, jid: string, canal?: object, canalTipo?: string}} contexto -
 *   Datos para las herramientas. `canal` es el adaptador ({enviarTexto, enviarImagen})
 *   y `canalTipo` es 'instagram' (default) u otro canal.
...
export async function responder(jid, textoUsuario, contexto) {
  ...
  if (!sesion) {
    sesion = {
      mensajes: [{ role: 'system', content: construirSystemPrompt(contexto?.canalTipo, personaParaChat(jid).nombre) }],
      ultimaActividad: Date.now(),
      canalTipo: contexto?.canalTipo || 'instagram'
    };
    historiales.set(jid, sesion);
  }
  sesion.ultimaActividad = Date.now();
